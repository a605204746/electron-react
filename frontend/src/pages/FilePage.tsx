...
export default function FilePage() {
  ...
  const read = async () => {
    ...
    try {
      ...
      setError(e.message ?? tr.file.readFailed)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="page-enter" style={{ padding: 32 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{
          ...
          <span style={{ color: '#a78bfa', fontSize: 11, fontWeight: 600, letterSpacing: '0.09em' }}>{tr.file.badge}</span>
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <ReadBtn label={tr.file.readBtn} onClick={read} loading={loading} disabled={!path.trim()} />
        </div>
      </div>
      ...
      {error && (
        <div style={{
          ...
          <div>
            <div style={{ color: '#f87171', fontSize: 13, fontWeight: 500, marginBottom: 3 }}>{tr.file.readFailed}</div>
      ...
      {content !== null && (
        <div style={{
          ...
          {/* Terminal titlebar */}
          <div style={{
            padding: '9px 14px', borderBottom: `1px solid ${t.borderSubtle}`,
            ...
            <span style={{ color: t.textFaint, fontSize: 10, fontFamily: 'monospace' }}>
              {tr.file.lineChar(content.split('\n').length, content.length)}
            </span>
          </div>
          <pre style={{
            margin: 0, padding: '16px 20px',
            color: t.textSub, fontSize: 12, lineHeight: 1.75,
      ...
      {content === null && !error && (
        <div style={{
          ...
          <div style={{ color: t.textFaint, fontSize: 13 }}>{tr.file.emptyHint}</div>
        </div>
      )}
...
function PathInput({ value, onChange, onEnter }: { value: string; onChange: (v: string) => void; onEnter: () => void }) {
  ...
  return (
    <div style={{
      ...
      background: t.bgDeep,
      border: `1px solid ${focus ? 'rgba(167,139,250,0.4)' : t.border}`,
      borderRadius: 8, padding: '0 12px',
      transition: 'border-color 0.15s',
    }}>
      <FolderOpenOutlined style={{ color: focus ? '#a78bfa' : t.textFaint, fontSize: 14, transition: 'color 0.15s', flexShrink: 0 }} />
      <input
        value={value}
...
function ReadBtn({ label, onClick, loading, disabled }: { label: string; onClick: () => void; loading: boolean; disabled: boolean }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${!disabled ? 'rgba(167,139,250,0.4)' : 'rgba(167,139,250,0.15)'}`,
        borderRadius: 8, padding: '0 18px',
        color: !disabled ? '#a78bfa' : 'rgba(167,139,250,0.35)',
        fontSize: 13, fontWeight: 500,
        cursor: disabled ? 'default' : 'pointer',
        transition: 'all 0.15s', justifyContent: 'center', outline: 'none',
      }}
    >
